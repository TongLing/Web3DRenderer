...
function walkFileTree(item,path) {
  ...
  if(item.isFile) {
    ...
    item.file(function(file) {                // this is also asynchronous ... (making everything complicate)
      if(file.name.match(/\.(jscad|js|scad|obj|stl|amf|gcode|vrl|stp)$/i)) {   // FIXME now all files OpenJSCAD can handle
...
/***********************/
  if(!file.name.match(/\.(jscad|js|scad|stl|obj|amf|gcode|vrl|stp)$/i)) { // FIXME where is the list?
...
/*********************/
    var e = fn.toLowerCase().match(/\.(jscad|js|scad|stl|obj|amf|gcode|vrl|stp)$/i);
    e = RegExp.$1;
    if(e == 'amf') {
